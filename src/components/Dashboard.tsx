...
const Dashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* WhatsApp Popup - Mobile Only */}
      <a
        href="https://wa.me/2349131124859?text=Hi%2C%20I%20need%20help%20with%20delivery"
        target="_blank"
        rel="noopener noreferrer"
        className={`md:hidden fixed bottom-24 z-50 flex items-center transition-all duration-500 ease-out ${
          showWhatsAppPopup 
            ? 'left-5 opacity-100' 
            : '-left-20 opacity-0'
        }`}
        style={{
          animation: showWhatsAppPopup ? 'bounce 0.6s ease-out' : 'none'
        }}
      >
        ...
        <div className={`ml-2 bg-white rounded-lg px-3 py-2 shadow-lg max-w-[150px] transition-all duration-300 ${
          showWhatsAppPopup ? 'opacity-100 scale-100' : 'opacity-0 scale-95'
        }`}>
